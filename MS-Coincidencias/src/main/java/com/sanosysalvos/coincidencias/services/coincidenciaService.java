    ...
    public NotificacionDTO procesarNuevoReporte(ReporteDTO reporte) {
        ...
        for (reporteModel candidato : candidatos) {
            ...
            if (score >= MATCH_THRESHOLD) {
                reporteModel reporteEntidad = repoReportes.findById(reporte.getId()).orElse(null);
                if (reporteEntidad == null) return null;

                coincidenciaModel coincidencia = new coincidenciaModel();

                if ("PERDIDO".equalsIgnoreCase(reporte.getTipoReporte())) {
                    coincidencia.setReportePerdida(reporteEntidad);
                    coincidencia.setReporteEncontrado(candidato);
                } else {
                    coincidencia.setReportePerdida(candidato);
                    coincidencia.setReporteEncontrado(reporteEntidad);
                }
                ...
                java.util.UUID idUsuarioReportePerdida = guardada.getReportePerdida() != null
                        ? guardada.getReportePerdida().getIdUsuario()
                        : null;

                NotificacionDTO notificacion = NotificacionDTO.builder()
                        .idCoincidencia(guardada.getIdCoincidencia())
                        .idReportePerdida(guardada.getReportePerdida().getIdReporte())
                        .idReporteEncontrado(guardada.getReporteEncontrado().getIdReporte())
